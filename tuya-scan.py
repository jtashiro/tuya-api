...
def status_summary(status: dict, temperature_f: float | None = None) -> str:
    """Build a compact, human-readable status string from status codes, with Fahrenheit if present."""
    if not status:
        return "—"
    ...
    if temperature_f is not None:
        parts.append(f"temp_F={temperature_f:.1f}")
...
def main():
    ...
    for dev in devices:
        ...
        # Find temperature value and convert to Fahrenheit
        temperature_f = None
        ...
        rows.append({
            ...
            "Status": status_summary(status, temperature_f),
    ...
    print(table)
    # Print a summary of devices with temperature_f
    temp_devices = [r for r in rows if r["temperature_f"] is not None]
    if temp_devices:
        print("\nDevices with temperature readings (F):")
        for r in temp_devices:
            print(f"  {r['Name']}: {r['temperature_f']:.1f}°F")
